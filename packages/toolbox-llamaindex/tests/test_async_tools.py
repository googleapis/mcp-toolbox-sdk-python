...
class TestAsyncToolboxTool:
    ...
    async def test_toolbox_tool_bind_secure_params(self, toolbox_tool):
        ...
        with patch.object(
            ...
            new_llamaindex_tool = toolbox_tool.bind_secure_params({"api_key": "sec123"})
            mock_core_bind.assert_called_once_with({"api_key": "sec123"})
            assert isinstance(
                new_llamaindex_tool._AsyncToolboxTool__core_tool,
                ToolboxCoreTool,
            )
    ...
    async def test_toolbox_tool_bind_secure_param(self, toolbox_tool):
        ...
        new_llamaindex_tool = toolbox_tool.bind_secure_param("api_key", "sec123")
        assert isinstance(
            new_llamaindex_tool._AsyncToolboxTool__core_tool, ToolboxCoreTool
        )
        assert (
            new_llamaindex_tool._AsyncToolboxTool__core_tool._bound_secure_params
            == {"api_key": "sec123"}
        )
